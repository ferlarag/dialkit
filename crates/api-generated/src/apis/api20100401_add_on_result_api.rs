...
) -> Result<(), Error<DeleteRecordingAddOnResultError>> {
    let uri_str = format!(
...
pub async fn fetch_recording_add_on_result(
    ...
    models::ApiV2010AccountRecordingRecordingAddOnResult,
    Error<FetchRecordingAddOnResultError>,
> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/Recordings/{ReferenceSid}/AddOnResults/{Sid}.json",
        configuration.base_path,
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountRecordingRecordingAddOnResult`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountRecordingRecordingAddOnResult`"
                ))));
            }
        }
    } else {
...
pub async fn list_recording_add_on_result(
...
) -> Result<models::ListRecordingAddOnResultResponse, Error<ListRecordingAddOnResultError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListRecordingAddOnResultResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListRecordingAddOnResultResponse`"
                ))));
            }
        }
    } else {
