...
///
pub async fn list_usage_record_last_month(
    configuration: &configuration::Configuration,
    params: ListUsageRecordLastMonthParams,
) -> Result<models::ListUsageRecordLastMonthResponse, Error<ListUsageRecordLastMonthError>> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListUsageRecordLastMonthResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListUsageRecordLastMonthResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListUsageRecordLastMonthError> = serde_json::from_str(&content).ok();
