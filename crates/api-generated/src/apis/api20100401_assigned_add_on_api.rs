...
pub async fn create_incoming_phone_number_assigned_add_on(
    ...
    models::ApiV2010AccountIncomingPhoneNumberIncomingPhoneNumberAssignedAddOn,
...
> {
    let uri_str = format!(
        ...
        ResourceSid = crate::apis::urlencode(params.resource_sid)
    );
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountIncomingPhoneNumberIncomingPhoneNumberAssignedAddOn`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountIncomingPhoneNumberIncomingPhoneNumberAssignedAddOn`"
                ))));
            }
        }
    } else {
...
) -> Result<(), Error<DeleteIncomingPhoneNumberAssignedAddOnError>> {
    let uri_str = format!(
...
pub async fn list_incoming_phone_number_assigned_add_on(
    ...
    models::ListIncomingPhoneNumberAssignedAddOnResponse,
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListIncomingPhoneNumberAssignedAddOnResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListIncomingPhoneNumberAssignedAddOnResponse`"
                ))));
            }
        }
    } else {
