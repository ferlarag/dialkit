...
) -> Result<models::ApiV2010AccountShortCode, Error<FetchShortCodeError>> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountShortCode`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountShortCode`"
                ))));
            }
        }
    } else {
...
) -> Result<models::ApiV2010AccountShortCode, Error<UpdateShortCodeError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/SMS/ShortCodes/{Sid}.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
