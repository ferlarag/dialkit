...
///
pub async fn list_usage_record_this_month(
    configuration: &configuration::Configuration,
    params: ListUsageRecordThisMonthParams,
) -> Result<models::ListUsageRecordThisMonthResponse, Error<ListUsageRecordThisMonthError>> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListUsageRecordThisMonthResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListUsageRecordThisMonthResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListUsageRecordThisMonthError> = serde_json::from_str(&content).ok();
