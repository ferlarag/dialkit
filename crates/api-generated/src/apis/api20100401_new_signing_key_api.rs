...
pub async fn create_new_signing_key(
    configuration: &configuration::Configuration,
    params: CreateNewSigningKeyParams,
) -> Result<models::ApiV2010AccountNewSigningKey, Error<CreateNewSigningKeyError>> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountNewSigningKey`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountNewSigningKey`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateNewSigningKeyError> = serde_json::from_str(&content).ok();
