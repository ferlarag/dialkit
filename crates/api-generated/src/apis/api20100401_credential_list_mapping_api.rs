...
pub async fn create_sip_credential_list_mapping(
    ...
    models::ApiV2010AccountSipSipDomainSipCredentialListMapping,
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountSipSipDomainSipCredentialListMapping`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountSipSipDomainSipCredentialListMapping`"
                ))));
            }
        }
    } else {
...
) -> Result<models::ListSipCredentialListMappingResponse, Error<ListSipCredentialListMappingError>>
{
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/SIP/Domains/{DomainSid}/CredentialListMappings.json",
        configuration.base_path,
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListSipCredentialListMappingResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListSipCredentialListMappingResponse`"
                ))));
            }
        }
    } else {
