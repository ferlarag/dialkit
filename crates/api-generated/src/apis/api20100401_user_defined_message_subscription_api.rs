...
pub async fn create_user_defined_message_subscription(
    ...
    models::ApiV2010AccountCallUserDefinedMessageSubscription,
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountCallUserDefinedMessageSubscription`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountCallUserDefinedMessageSubscription`"
                ))));
            }
        }
    } else {
...
) -> Result<(), Error<DeleteUserDefinedMessageSubscriptionError>> {
    let uri_str = format!(
