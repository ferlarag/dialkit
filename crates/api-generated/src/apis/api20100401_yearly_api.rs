...
///
pub async fn list_usage_record_yearly(
    configuration: &configuration::Configuration,
    params: ListUsageRecordYearlyParams,
) -> Result<models::ListUsageRecordYearlyResponse, Error<ListUsageRecordYearlyError>> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListUsageRecordYearlyResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListUsageRecordYearlyResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListUsageRecordYearlyError> = serde_json::from_str(&content).ok();
