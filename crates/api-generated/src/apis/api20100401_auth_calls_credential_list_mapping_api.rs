...
pub async fn create_sip_auth_calls_credential_list_mapping(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountSipSipDomainSipAuthSipAuthCallsSipAuthCallsCredentialListMapping`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountSipSipDomainSipAuthSipAuthCallsSipAuthCallsCredentialListMapping`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateSipAuthCallsCredentialListMappingError> =
...
pub async fn delete_sip_auth_calls_credential_list_mapping(
    configuration: &configuration::Configuration,
    params: DeleteSipAuthCallsCredentialListMappingParams,
) -> Result<(), Error<DeleteSipAuthCallsCredentialListMappingError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DeleteSipAuthCallsCredentialListMappingError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
...
pub async fn fetch_sip_auth_calls_credential_list_mapping(
    configuration: &configuration::Configuration,
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<FetchSipAuthCallsCredentialListMappingError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
...
pub async fn list_sip_auth_calls_credential_list_mapping(
    configuration: &configuration::Configuration,
    params: ListSipAuthCallsCredentialListMappingParams,
) -> Result<
    models::ListSipAuthCallsCredentialListMappingResponse,
    Error<ListSipAuthCallsCredentialListMappingError>,
> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/SIP/Domains/{DomainSid}/Auth/Calls/CredentialListMappings.json",
        configuration.base_path,
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListSipAuthCallsCredentialListMappingResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListSipAuthCallsCredentialListMappingResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListSipAuthCallsCredentialListMappingError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
