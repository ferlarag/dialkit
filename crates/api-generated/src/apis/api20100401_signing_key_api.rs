...
) -> Result<(), Error<DeleteSigningKeyError>> {
    let uri_str = format!(
...
) -> Result<models::ApiV2010AccountSigningKey, Error<FetchSigningKeyError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/SigningKeys/{Sid}.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountSigningKey`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountSigningKey`"
                ))));
            }
        }
    } else {
...
pub async fn list_signing_key(
...
) -> Result<models::ListSigningKeyResponse, Error<ListSigningKeyError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListSigningKeyResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListSigningKeyResponse`"
                ))));
            }
        }
    } else {
...
) -> Result<models::ApiV2010AccountSigningKey, Error<UpdateSigningKeyError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/SigningKeys/{Sid}.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
