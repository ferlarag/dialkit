...
pub async fn create_incoming_phone_number_toll_free(
    ...
    models::ApiV2010AccountIncomingPhoneNumberIncomingPhoneNumberTollFree,
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountIncomingPhoneNumberIncomingPhoneNumberTollFree`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountIncomingPhoneNumberIncomingPhoneNumberTollFree`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateIncomingPhoneNumberTollFreeError> =
...
pub async fn list_incoming_phone_number_toll_free(
    ...
    models::ListIncomingPhoneNumberTollFreeResponse,
    Error<ListIncomingPhoneNumberTollFreeError>,
> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/IncomingPhoneNumbers/TollFree.json",
        configuration.base_path,
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListIncomingPhoneNumberTollFreeResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListIncomingPhoneNumberTollFreeResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListIncomingPhoneNumberTollFreeError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
