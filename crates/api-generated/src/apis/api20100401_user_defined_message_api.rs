...
pub async fn create_user_defined_message(
    ...
    params: CreateUserDefinedMessageParams,
) -> Result<models::ApiV2010AccountCallUserDefinedMessage, Error<CreateUserDefinedMessageError>> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountCallUserDefinedMessage`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountCallUserDefinedMessage`"
                ))));
            }
        }
    } else {
