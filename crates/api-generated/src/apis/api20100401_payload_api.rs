...
pub async fn delete_recording_add_on_result_payload(
    configuration: &configuration::Configuration,
    params: DeleteRecordingAddOnResultPayloadParams,
) -> Result<(), Error<DeleteRecordingAddOnResultPayloadError>> {
    let uri_str = format!(
        ...
        AddOnResultSid = crate::apis::urlencode(params.add_on_result_sid),
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<DeleteRecordingAddOnResultPayloadError> =
...
pub async fn fetch_recording_add_on_result_payload(
    configuration: &configuration::Configuration,
    params: FetchRecordingAddOnResultPayloadParams,
) -> Result<
    models::ApiV2010AccountRecordingRecordingAddOnResultRecordingAddOnResultPayload,
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountRecordingRecordingAddOnResultRecordingAddOnResultPayload`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountRecordingRecordingAddOnResultRecordingAddOnResultPayload`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FetchRecordingAddOnResultPayloadError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
...
pub async fn list_recording_add_on_result_payload(
    configuration: &configuration::Configuration,
    params: ListRecordingAddOnResultPayloadParams,
) -> Result<
    models::ListRecordingAddOnResultPayloadResponse,
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListRecordingAddOnResultPayloadResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListRecordingAddOnResultPayloadResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListRecordingAddOnResultPayloadError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
