...
pub async fn list_available_phone_number_shared_cost(
    ...
    models::ListAvailablePhoneNumberSharedCostResponse,
    Error<ListAvailablePhoneNumberSharedCostError>,
> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListAvailablePhoneNumberSharedCostResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListAvailablePhoneNumberSharedCostResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListAvailablePhoneNumberSharedCostError> =
