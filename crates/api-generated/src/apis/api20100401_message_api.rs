...
pub async fn create_message(
    ...
    if let Some(param_values) = params.persistent_action {
        for param_value in param_values {
            multipart_form_params.insert("PersistentAction", param_value.to_string());
        }
    }
    ...
    if let Some(param_value) = params.message_intent {
        multipart_form_params.insert("MessageIntent", param_value.to_string());
    }
    ...
    if let Some(param_values) = params.media_url {
        for param_value in param_values {
            multipart_form_params.insert("MediaUrl", param_value.to_string());
        }
    }
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountMessage`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountMessage`"
                ))));
            }
        }
    } else {
...
pub async fn list_message(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListMessageResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListMessageResponse`"
                ))));
            }
        }
    } else {
