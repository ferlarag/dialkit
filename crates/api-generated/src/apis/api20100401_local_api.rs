...
///
pub async fn list_available_phone_number_local(
    configuration: &configuration::Configuration,
    params: ListAvailablePhoneNumberLocalParams,
) -> Result<models::ListAvailablePhoneNumberLocalResponse, Error<ListAvailablePhoneNumberLocalError>>
{
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListAvailablePhoneNumberLocalResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListAvailablePhoneNumberLocalResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListAvailablePhoneNumberLocalError> =
