...
///
pub async fn create_incoming_phone_number_mobile(
    configuration: &configuration::Configuration,
    params: CreateIncomingPhoneNumberMobileParams,
) -> Result<
    models::ApiV2010AccountIncomingPhoneNumberIncomingPhoneNumberMobile,
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountIncomingPhoneNumberIncomingPhoneNumberMobile`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountIncomingPhoneNumberIncomingPhoneNumberMobile`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateIncomingPhoneNumberMobileError> =
...
) -> Result<models::ListIncomingPhoneNumberMobileResponse, Error<ListIncomingPhoneNumberMobileError>>
{
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/IncomingPhoneNumbers/Mobile.json",
        configuration.base_path,
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListIncomingPhoneNumberMobileResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListIncomingPhoneNumberMobileResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListIncomingPhoneNumberMobileError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
