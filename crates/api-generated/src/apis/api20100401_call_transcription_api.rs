...
pub struct CreateRealtimeTranscriptionParams {
    ...
    pub transcription_configuration_id: Option<String>,
...
) -> Result<models::ApiV2010AccountCallRealtimeTranscription, Error<CreateRealtimeTranscriptionError>>
{
    ...
    if let Some(param_value) = params.transcription_configuration_id {
        multipart_form_params.insert("TranscriptionConfigurationId", param_value.to_string());
    }
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountCallRealtimeTranscription`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountCallRealtimeTranscription`"
                ))));
            }
        }
    } else {
