...
pub enum UpdateAccountError {
    Status409(models::UpdateAccount409Response),
    UnknownValue(serde_json::Value),
}
...
) -> Result<models::ApiV2010Account, Error<CreateAccountError>> {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010Account`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010Account`"
                ))));
            }
        }
    } else {
...
) -> Result<models::ApiV2010Account, Error<FetchAccountError>> {
    let uri_str = format!(
...
pub async fn list_account(
...
) -> Result<models::ListAccountResponse, Error<ListAccountError>> {
    let uri_str = format!("{}/2010-04-01/Accounts.json", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListAccountResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListAccountResponse`"
                ))));
            }
        }
    } else {
...
) -> Result<models::ApiV2010Account, Error<UpdateAccountError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{Sid}.json",
        configuration.base_path,
        Sid = crate::apis::urlencode(params.sid)
