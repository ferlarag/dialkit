...
pub struct ListNotificationParams {
    ...
    /// Only show notifications for the specified date, formatted as `YYYY-MM-DD`. You can also specify an inequality, such as `<=YYYY-MM-DD` for messages logged at or before midnight on a date, or `>=YYYY-MM-DD` for messages logged at or after midnight on a date.
    pub message_date_less_than: Option<chrono::NaiveDate>,
    /// Only show notifications for the specified date, formatted as `YYYY-MM-DD`. You can also specify an inequality, such as `<=YYYY-MM-DD` for messages logged at or before midnight on a date, or `>=YYYY-MM-DD` for messages logged at or after midnight on a date.
    pub message_date_greater_than: Option<chrono::NaiveDate>,
...
) -> Result<models::ApiV2010AccountNotificationInstance, Error<FetchNotificationError>> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountNotificationInstance`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountNotificationInstance`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FetchNotificationError> = serde_json::from_str(&content).ok();
...
pub async fn list_notification(
...
) -> Result<models::ListNotificationResponse, Error<ListNotificationError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListNotificationResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListNotificationResponse`"
                ))));
            }
        }
    } else {
