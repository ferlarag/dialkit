...
) -> Result<models::ApiV2010AccountKey, Error<FetchKeyError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/Keys/{Sid}.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountKey`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountKey`"
                ))));
            }
        }
    } else {
...
pub async fn list_key(
...
) -> Result<models::ListKeyResponse, Error<ListKeyError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListKeyResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListKeyResponse`"
                ))));
            }
        }
    } else {
...
) -> Result<models::ApiV2010AccountKey, Error<UpdateKeyError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/Keys/{Sid}.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
