...
) -> Result<models::ApiV2010AccountQueue, Error<CreateQueueError>> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountQueue`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountQueue`"
                ))));
            }
        }
    } else {
...
) -> Result<models::ApiV2010AccountQueue, Error<FetchQueueError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/Queues/{Sid}.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
...
pub async fn list_queue(
...
) -> Result<models::ListQueueResponse, Error<ListQueueError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/Queues.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid)
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListQueueResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListQueueResponse`"
                ))));
            }
        }
    } else {
...
) -> Result<models::ApiV2010AccountQueue, Error<UpdateQueueError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/Queues/{Sid}.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
