...
) -> Result<models::ApiV2010AccountQueueMember, Error<FetchMemberError>> {
    let uri_str = format!(
        ...
        QueueSid = crate::apis::urlencode(params.queue_sid),
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountQueueMember`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountQueueMember`"
                ))));
            }
        }
    } else {
...
pub async fn list_member(
...
) -> Result<models::ListMemberResponse, Error<ListMemberError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListMemberResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListMemberResponse`"
                ))));
            }
        }
    } else {
...
) -> Result<models::ApiV2010AccountQueueMember, Error<UpdateMemberError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/Queues/{QueueSid}/Members/{CallSid}.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
