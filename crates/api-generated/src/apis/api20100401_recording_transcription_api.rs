...
pub async fn fetch_recording_transcription(
    ...
    models::ApiV2010AccountRecordingRecordingTranscription,
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountRecordingRecordingTranscription`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountRecordingRecordingTranscription`"
                ))));
            }
        }
    } else {
...
pub async fn list_recording_transcription(
...
) -> Result<models::ListRecordingTranscriptionResponse, Error<ListRecordingTranscriptionError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListRecordingTranscriptionResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListRecordingTranscriptionResponse`"
                ))));
            }
        }
    } else {
