...
pub async fn create_sip_ip_access_control_list(
    ...
    models::ApiV2010AccountSipSipIpAccessControlList,
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountSipSipIpAccessControlList`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountSipSipIpAccessControlList`"
                ))));
            }
        }
    } else {
...
) -> Result<(), Error<DeleteSipIpAccessControlListError>> {
    let uri_str = format!(
...
) -> Result<models::ApiV2010AccountSipSipIpAccessControlList, Error<FetchSipIpAccessControlListError>>
{
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/SIP/IpAccessControlLists/{Sid}.json",
        configuration.base_path,
...
pub async fn list_sip_ip_access_control_list(
...
) -> Result<models::ListSipIpAccessControlListResponse, Error<ListSipIpAccessControlListError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/SIP/IpAccessControlLists.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid)
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListSipIpAccessControlListResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListSipIpAccessControlListResponse`"
                ))));
            }
        }
    } else {
