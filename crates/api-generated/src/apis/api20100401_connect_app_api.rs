...
pub async fn delete_connect_app(
    configuration: &configuration::Configuration,
    params: DeleteConnectAppParams,
) -> Result<(), Error<DeleteConnectAppError>> {
    let uri_str = format!(
...
pub async fn fetch_connect_app(
    configuration: &configuration::Configuration,
    params: FetchConnectAppParams,
) -> Result<models::ApiV2010AccountConnectApp, Error<FetchConnectAppError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/ConnectApps/{Sid}.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountConnectApp`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountConnectApp`"
                ))));
            }
        }
    } else {
...
pub async fn list_connect_app(
...
) -> Result<models::ListConnectAppResponse, Error<ListConnectAppError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListConnectAppResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListConnectAppResponse`"
                ))));
            }
        }
    } else {
...
pub async fn update_connect_app(
    configuration: &configuration::Configuration,
    params: UpdateConnectAppParams,
) -> Result<models::ApiV2010AccountConnectApp, Error<UpdateConnectAppError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/ConnectApps/{Sid}.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
    ...
    if let Some(param_values) = params.permissions {
        for param_value in param_values {
            multipart_form_params.insert("Permissions", param_value.to_string());
        }
    }
