...
) -> Result<models::ListUsageRecordDailyResponse, Error<ListUsageRecordDailyError>> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListUsageRecordDailyResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListUsageRecordDailyResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListUsageRecordDailyError> = serde_json::from_str(&content).ok();
