...
) -> Result<models::ListDependentPhoneNumberResponse, Error<ListDependentPhoneNumberError>> {
    let uri_str = format!(
        ...
        AddressSid = crate::apis::urlencode(params.address_sid)
    );
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListDependentPhoneNumberResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListDependentPhoneNumberResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListDependentPhoneNumberError> = serde_json::from_str(&content).ok();
