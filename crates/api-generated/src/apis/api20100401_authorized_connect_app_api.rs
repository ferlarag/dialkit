...
pub async fn fetch_authorized_connect_app(
    configuration: &configuration::Configuration,
    params: FetchAuthorizedConnectAppParams,
) -> Result<models::ApiV2010AccountAuthorizedConnectApp, Error<FetchAuthorizedConnectAppError>> {
    let uri_str = format!(
        ...
        ConnectAppSid = crate::apis::urlencode(params.connect_app_sid)
    );
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountAuthorizedConnectApp`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountAuthorizedConnectApp`"
                ))));
            }
        }
    } else {
...
pub async fn list_authorized_connect_app(
...
) -> Result<models::ListAuthorizedConnectAppResponse, Error<ListAuthorizedConnectAppError>> {
    let uri_str = format!(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListAuthorizedConnectAppResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListAuthorizedConnectAppResponse`"
                ))));
            }
        }
    } else {
