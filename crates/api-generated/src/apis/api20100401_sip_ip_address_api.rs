...
pub async fn create_sip_ip_address(
    ...
    models::ApiV2010AccountSipSipIpAccessControlListSipIpAddress,
...
> {
    let uri_str = format!(
        ...
        IpAccessControlListSid = crate::apis::urlencode(params.ip_access_control_list_sid)
    );
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ApiV2010AccountSipSipIpAccessControlListSipIpAddress`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ApiV2010AccountSipSipIpAccessControlListSipIpAddress`"
                ))));
            }
        }
    } else {
...
pub async fn list_sip_ip_address(
...
) -> Result<models::ListSipIpAddressResponse, Error<ListSipIpAddressError>> {
    let uri_str = format!(
        "{}/2010-04-01/Accounts/{AccountSid}/SIP/IpAccessControlLists/{IpAccessControlListSid}/IpAddresses.json",
        configuration.base_path,
        AccountSid = crate::apis::urlencode(params.account_sid),
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListSipIpAddressResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListSipIpAddressResponse`"
                ))));
            }
        }
    } else {
