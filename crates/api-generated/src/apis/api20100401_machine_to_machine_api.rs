...
pub async fn list_available_phone_number_machine_to_machine(
    ...
    models::ListAvailablePhoneNumberMachineToMachineResponse,
    Error<ListAvailablePhoneNumberMachineToMachineError>,
> {
    let uri_str = format!(
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::ListAvailablePhoneNumberMachineToMachineResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::ListAvailablePhoneNumberMachineToMachineResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListAvailablePhoneNumberMachineToMachineError> =
